...
def build_parser():
    ...
    parser.add_argument('--participant_label', '--participant-label',
                        help="(optional) The name/label of the subject to be processed (ex. sub-XXXXX)", type=str)
    parser.add_argument('--session_id', '--session-id',
                        help="(optional) The name of a specific session to be processed (ex. ses-V02)", type=str)
    parser.add_argument('--interval',
                        help="(optional) The label to correct or not the uneven sampling interval",
                        type=str, choices=['raw', 'corrected'])
    parser.add_argument('--pa_measure', '--pa-measure',
                        help="(optional) The computedQttyOption value (acceleration or jerk)",
                        type=str, choices=['acceleration', 'jerk'])
    parser.add_argument('--pa_side', '--pa-side',
                        help="(optional) which leg to calculate the physical activity level (Left/L or Right/R)",
                        type=str, choices=['Left', 'L', 'Right', 'R'])
    parser.add_argument('--entropy_type', '--entropy-type',
                        help="(optional) Entropy type (SampEn or FuzzEn)",
                        type=str, choices=['SampEn', 'FuzzEn'])
    parser.add_argument('--entropy_measure', '--entropy-measure',
                        help="(optional) Measure to calculate an entropy (avgacc or pkacc)",
